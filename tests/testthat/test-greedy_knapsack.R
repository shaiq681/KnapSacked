...
test_that("Function return correct results.", {
  ...
  expect_gte(round(gk$value), 15000)
  expect_true(all(round(gk$elements) %in% c(1:8)))

  gk <- greedy_knapsack(x = knapsack_objects[1:12,], W = 3500)
  expect_gte(round(gk$value), 15428)
  expect_true(all(round(gk$elements) %in% c(1:12)))
  ...
  expect_equal(round(gk$value), 15428)
  expect_true(all(round(gk$elements) %in% c(1:16)))

  st <- system.time(gk <- greedy_knapsack(x = knapsack_objects[1:16,], W = 2000))
  expect_true(as.numeric(st)[2] <= 0.1)
